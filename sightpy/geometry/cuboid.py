...
class Cuboid_Collider(Collider):
    ...
    def intersect(self, O, D):
        ...
        tmin = np.maximum(
            np.maximum(np.minimum(t1, t2), np.minimum(t3, t4)), np.minimum(t5, t6)
        )
        tmax = np.minimum(
            np.minimum(np.maximum(t1, t2), np.maximum(t3, t4)), np.maximum(t5, t6)
        )
        ...
        return np.select(
            ...
            [
                FARAWAY,
                [tmax, np.tile(UPDOWN, tmin.shape)],
