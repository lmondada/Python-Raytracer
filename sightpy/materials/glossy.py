    ...
    def get_color(self, scene, ray, hit):
        ...
        for light in scene.Light_list:
            ...
            if self.roughness != 0.0:
                ...
                cosθ = np.clip(V.dot(H), 0.0, 1.0)
                F = F0 + (1.0 - F0) * (1.0 - cosθ) ** 5

                # Phong shading (specular highlight)
                a = 2.0 / (self.roughness ** 2.0) - 2.0
                Dphong = (
                    np.power(np.clip(N.dot(H), 0.0, 1.0), a) * (a + 2.0) / (2.0 * np.pi)
                )
                ...
                color += (
                    F
                    * Dphong
                    / (4.0 * np.clip(N.dot(V) * NdotL, 0.001, 1.0))
                    * seelight
                    * lv
                    * self.spec_coeff
                )
