...
class Diffuse(Material):
    ...
    def get_color(self, scene, ray, hit):
        ...
        if ray.diffuse_reflections < 1:
            ...
            pdf2 = spherical_caps_pdf(
                size, nudged_repeated, scene.importance_sampled_list
            )
            ...
            color_temp = (
                color_temp * NdotL / PDF_val / (np.pi)
            )  #  diff_color/np.pi = Lambertian BRDF
            color += diff_color * color_temp.reshape(
                N.shape()[0], self.diffuse_rays
            ).mean(axis=1)
            ...
            NdotL = np.clip(N.dot(ray_dir), 0.0, 1.0)
            color_temp = diff_color * get_raycolor(
                Ray(
                    nudged,
                    ray_dir,
                    ray.depth + 1,
